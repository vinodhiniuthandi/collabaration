...
app.factory('UserService',['$http', '$q','$rootScope', function($http,$q,$rootScope){
	...
	var BASE_URL="http://localhost:8085/CollaborationBackEnd"
		...
		accept: function(user,id){
			...
			return $http.put(BASE_URL+'/useraccept/'+user.name,user)
			.then(
					function(response){
						return response.data;
					},
		...
		reject: function(user,id){
			...
			return $http.put(BASE_URL+'/userreject/'+user.name,user)
			.then(
					function(response){
						return response.data;
					},
		...
		myprofile: function(id){
			...
			return $http.get(BASE_URL+'/user/'+name)
			.then(
					function(response){
						return response.data;
					},
		...
		updateUser: function(user,id){
			return $http.put(BASE_URL+'/user/'+name,user)
			.then(
					function(response){
						return response.data;
					},
	...
	deleteUser: function(id){
		return $http,delete(BASE_URL+'/user/'+name)
		.then(
				function(response){
					return response.data;
